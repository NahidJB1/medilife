...
function loadPatientHistory(typeFilter, containerId, fallbackDrDetails, uploadedByFilter) {
    ...
    .then(data => {
        ...
        data.forEach(d => {
            ...
            // --- NEW: Privacy Menu HTML (Only for Reports) ---
            let privacyMenuHtml = '';
            if (typeFilter === 'Report' && d.id) {
                // Ensure the report has an ID to update it
                privacyMenuHtml = `
                    <div class="menu-container">
                        <button class="btn-dots" onclick="togglePrivacyMenu(event, ${d.id})"><i class="fas fa-ellipsis-v"></i></button>
                        <div id="privacy-menu-${d.id}" class="privacy-dropdown">
                            <div class="dropdown-header">Document Visibility</div>
                            <div class="dropdown-item" onclick="updateReportPrivacy(${d.id}, 'all')">
                                <i class="fas fa-globe" style="color:#10B981; width:20px;"></i> Patient & Doctors
                            </div>
                            <div class="dropdown-item" onclick="updateReportPrivacy(${d.id}, 'doctors_only')">
                                <i class="fas fa-user-md" style="color:#EF4444; width:20px;"></i> Doctors Only
                            </div>
                        </div>
                    </div>
                `;
...
// --- NEW: Privacy Menu Logic ---
function togglePrivacyMenu(event, reportId) {
    event.stopPropagation(); // Prevent immediate closing
    // Close any currently open menus first
    document.querySelectorAll('.privacy-dropdown.show').forEach(menu => {
        if (menu.id !== `privacy-menu-${reportId}`) menu.classList.remove('show');
    });
    // Toggle the targeted menu
    const menu = document.getElementById(`privacy-menu-${reportId}`);
    if (menu) menu.classList.toggle('show');
}

// Close dropdowns when clicking anywhere else on the screen (Mobile Friendly)
...
function updateReportPrivacy(reportId, privacyLevel) {
    const fd = new FormData();
    ...
    .then(data => {
        if(data.status === 'success') {
            showToast(privacyLevel === 'doctors_only' ? 'Restricted to Doctors Only' : 'Visible to Patient & Doctors');
            // Hide the menu after selection
            document.getElementById(`privacy-menu-${reportId}`).classList.remove('show');
        } else {
